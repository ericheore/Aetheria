...
import { Entity, LineStyle, GraphNode } from '../types';
import EntityEditor from '../components/EntityEditor';
import TagFilter from '../components/TagFilter';
import { useGraphPhysics } from '../hooks/useGraphPhysics';
import { Search, ZoomIn, ZoomOut, Maximize, Link as LinkIcon, Sliders, Plus, RotateCcw, RotateCw, X, Edit3, Trash2, Palette, ArrowRight, Focus, Target, Check, Edit2, ChevronDown, AlertTriangle } from 'lucide-react';
        ...
        <div className="h-full flex flex-col relative bg-slate-50 overflow-hidden" ref={containerRef}>
                        ...
                        <div className="flex items-center gap-1">
                             ...
                             <div ref={relInputContainerRef} className={`relative transition-all duration-300 ease-in-out ${isConnectMode ? 'w-48 opacity-100 ml-1' : 'w-0 opacity-0 overflow-hidden'}`}>
                                <div className="relative flex items-center">
                                    <input type="text" value={defaultRelLabel} onChange={(e) => { setDefaultRelLabel(e.target.value); setShowRelSuggestions(true); }} onFocus={() => setShowRelSuggestions(true)} placeholder={I18N.ph_type[language]} className="w-full pl-2 pr-6 py-1 text-xs border border-indigo-200 rounded bg-indigo-50/50 focus:bg-white focus:border-indigo-400 outline-none text-indigo-800 placeholder:text-indigo-300" />
            ...
            <div className="absolute bottom-6 left-6 z-10 flex gap-2 pointer-events-auto">
                 <div className="bg-white/90 backdrop-blur p-1 rounded-xl border border-gray-200 shadow-sm flex flex-col text-slate-600">
                     <button onClick={() => transformRef.current.k *= 1.2} className="p-2 hover:bg-slate-100 rounded-lg transition-colors" title={I18N.zoom_in[language]}><ZoomIn className="w-5 h-5" /></button>
                     <button onClick={() => transformRef.current.k /= 1.2} className="p-2 hover:bg-slate-100 rounded-lg transition-colors" title={I18N.zoom_out[language]}><ZoomOut className="w-5 h-5" /></button>
                     <div className="h-px w-6 bg-slate-200 mx-auto my-1"></div>
                     <button onClick={handleReset} className="p-2 hover:bg-slate-100 rounded-lg transition-colors" title={I18N.reset_view[language]}><Maximize className="w-5 h-5" /></button>
                 </div>
            </div>
